...
class arm_bi(gym.Env):
    ...
    def __init__(self, sutton_barto_reward: bool = False, render_mode: str | None = None):
        ...
        self.observation_space = spaces.Box(
            low=-np.inf, high=np.inf, shape=(22,), dtype=np.float32 #TODO obs有改，這邊之後要調整一下
        )
    ...
    def step(self, action):
        ...
        # 5. 計算獎勵 (Reward) 與 終止條件 (Done / Terminated)      TODO bi的reward還沒有調整，之後要調整
        observation = self._get_observation()
        # tip_pos = p.getLinkState(self.arm,9)[0]
        grap_pos = p.getLinkState(self.arm,11)[0]
        bi_pos,_ = p.getBasePositionAndOrientation(self.bi)
        pos1 = np.array(grap_pos)
        pos2 = np.array(bi_pos)
    ...
    def reset(
        ...
        *,
        ...
        self.pre_distance = np.linalg.norm(
            np.array(grap_pos)-np.array(bi_pos)
        )
    ...
    def set_up(self):
        ...
        bi_x =random.uniform(0.5,0.8)*random.choice([-1,1]) 
        bi_y =random.uniform(0.5,0.8)*random.choice([-1,1]) 
        respon_area_bi = [bi_x, bi_y, 1]
        bi_col = p.createCollisionShape(p.GEOM_BOX, halfExtents=[0.2, 0.2, 0.2])
        bi_vis = p.createVisualShape(p.GEOM_BOX, halfExtents=[0.2, 0.2, 0.2], rgbaColor=[0.2, 0.2, 0.8, 1]) # 藍色
        biId = p.createMultiBody(baseMass=0, baseCollisionShapeIndex=bi_col, baseVisualShapeIndex=bi_vis, basePosition=respon_area_bi)
...
#````````````````````````````````````````````````````````````
        ...
        for a,j,v,stable_pos in action_joinID_maxV:
    ...
    def _get_observation(self):
        ...
        # 收集各物件 XYZ 座標
        bi_pos, _ = p.getBasePositionAndOrientation(self.biId)
        grap_pos = p.getLinkState(self.arm,11)[0]
        joint_position = []
        joint_velocity = []
        ...
        relative_pos = np.array(bi_pos) - np.array(grap_pos)
